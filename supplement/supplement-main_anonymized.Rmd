...
output:
  bookdown::pdf_document2:
    includes:
      ...
      before_body: cover_anonymized.tex
...
Note: This is a fully reproducible manuscript, written in RMarkdown using the "knitr" and "bookdown" packages for R [@xie2016; @xie2021; @xie2024]. The full, editable script for all analyses, figures, and tables (here and in the main text) is available in the .Rmd file that generated this document (supplement-main.Rmd), which can be found in the following GitHub repository (anonymized for peer review): https://anonymous.4open.science/r/baby_mental_life_ms-C261/.
...
Methods, inclusion/exclusion criteria, and analyses marked as "preregistered" were preregistered on the Open Science Framework (OSF) website (anonymized links for peer review, Study 1: \url{https://osf.io/e6ajh/?view_only=f2539dbce3664172907aa7a381f62d34}; Study 2: \url{https://osf.io/j72dg/?view_only=7e4c01fec32f40a3b13c0ebc2b7ced18}; Study 3: \url{https://osf.io/xh8ce/?view_only=56649aff37aa4f1db2dc06b2e69576f4)}). 

All materials used in these studies are available in the following OSF repository: \url{https://osf.io/xrznd/?view_only=d9eb55e46028453cbd90534a003d3f52} (anonymized link for peer review).

All studies were approved by [IRB information omitted for anonymous review]. 
...
Traditional exploratory factor analysis techniques are not equipped to account for within-participant variance alongside between-participant variance. To conduct the EFAs reported in Results, we proceeded as if each participant's ratings of the 60 mental capacities for each of the three target ages were a distinct source of data; in other words, as if we had 903 participants assessing one target age each, instead of 301 participants assessing three target ages each. See [@weisman.etal2017a; @weisman.etal2021], for similar treatments of within-subjects data in the context of EFA; but see also [@owen.wang2016] for a bi-cross-validation technique, which might be adapted to address this repeated-measures design by structuring cross-validation folds to exclude all observations from a given participant within a given fold. (We thank a helpful reviewer for this suggestion.)

We used Pearson correlations to obtain minimal residual solutions and applied an oblique transformation (oblimin) to the resulting factor structures, using the "psych" package for R [@revelle2021]. 
...
```{r fig01, fig.cap="Study 1, factor solution suggested by parallel analysis. (Identical to Figure 1 in main text.)", include = F}
knitr::include_graphics("../outputs/fig01.pdf")
```

```{r figS01 code, fig.width = 8, fig.asp = 0.8}
# custom density function with adjustable smoothing
density_fn <- function(data, mapping, adj_val = 3, ...) {
  filtered_data <- data %>%
    filter(!is.na(factor))

  ggplot(filtered_data, mapping) +
    geom_density(
      aes(color = factor, fill = factor),
      alpha = 0.2, adjust = adj_val, inherit.aes = TRUE, ...)  # adjust controls smoothness
}

# custom points function with horizontal line
points_fn <- function(data, mapping, ...) {
  ggplot(data, mapping) +
    ...
    geom_point(
      aes(color = factor, fill = factor),
      alpha = 0.9, size = 1, inherit.aes = TRUE, ...)
}
...
ggpairs(
  ...
  s1_efa_par$loadings[] %>%
    ...
    mutate(
      # a priori sorting of capacities (for Studies 2-3)
      factor = case_when(
        capacity %in% c(
    ...
    mutate(capacity = gsub("_", " ", capacity)) %>%
    select(MR2, MR4, MR3, MR1, everything()) %>%
    rename(`Cognition and control` = MR1,
           `Bodily sensation` = MR2,
           `Social connection` = MR3,
           `Negative affect` = MR4),
  aes(color = factor, fill = factor),
  columns = 1:4,
  # upper = list(continuous = text_fn_filtered),
  upper = list(continuous = points_fn),
  lower = list(continuous = points_fn),
  # diag = list(continuous = ggally_densityDiag),
  diag = list(continuous = density_fn),
  # diag = "blank",
  showStrips = TRUE
) +
  geom_vline(xintercept = 0, lty = 2, color = "black", size = 0.3) +
  # geom_hline(yintercept = 0, lty = 2, color = "black", size = 0.3) +
  scale_color_viridis_d(
    name = "Domain (Studies 2-3):",
    end = 0.9, option = "viridis",
    na.value = "gray80",
    labels = c(
      "Bodily sensation", "Negative affect", 
      "Social connection", "Cognition and control", 
      "Not used in Studies 2-3"),
  ...
  # labs(color = "Domain (Studies 2-3):", fill = "Domain (Studies 2-3):") +
  theme_minimal() +
...
```{r figS01, fig.cap="Study 1, scatterplot matrix ('pairs plot') visualizing the relationships among factor loadings and the distributions of factor loadings across items for the four-factor solution suggested by parallel analysis (loadings presented in Figure 1). Along the diagonal are density plots showing, for each of the four factors, the distribution of factor loadings for the subset of capacities later used to represent each 'domain' in Studies 2-3. Panels in the upper and lower triangles are scatterplots of factor loadings on each pair of factors, including all of the capacities included in Study 1.", include = T}
knitr::include_graphics("../outputs/figS01.pdf")
```


```{r figS02 function, fig.width = 10}
figS02_fun <- function(domain1, domain2) {
  
  domain1 <- sym(domain1)
  domain2 <- sym(domain2)
  
  d <- s1_efa_par$loadings[] %>%
    data.frame() %>%
    rownames_to_column("capacity") %>%
    mutate(
      factor = case_when(
  ...
  p <- d %>%
    ...
    scale_color_viridis_d(
      name = "Domain (Studies 2-3):",
      end = 0.9, option = "viridis", na.value = "gray70",
      labels = c("Bodily sensation", "Negative affect", 
                 "Social connection", "Cognition and control",
                 "Not included in Studies 2-3")
    ) +
...
```{r figS02 code, fig.width = 10}
for(i in c("Bodily sensation", "Negative affect", 
           "Social connection", "Cognition and control")) {
  for(j in c("Bodily sensation", "Negative affect", 
             "Social connection", "Cognition and control")) {
    if(i != j) {
      figS02_fun(i, j)
      
      # save plot
      if (resave_plots == T | !file.exists(
        paste0("../outputs/figS02_",
               gsub(" ", "-", tolower(i)), "_",
               gsub(" ", "-", tolower(j)),
               ".pdf"))) {
        ggsave(
...
```{r figS02, fig.cap="Study 1, scatter plot visualizing the relationship between factor loadings on the factors we have labeled 'bodily sensation' and 'negative affect' in the factor solution suggested by parallel analysis (loadings presented in Figure 1). The subset of capacities used in Studies 2-3 are color-coded according to the domain they were selected to represent in those studies.", include = T}
knitr::include_graphics("../outputs/figS02_bodily-sensation_negative-affect.pdf")
```

```{r figS03, fig.cap="Study 1, scatter plot visualizing the relationship between factor loadings on the factors we have labeled 'bodily sensation' and 'social connection' in the factor solution suggested by parallel analysis (loadings presented in Figure 1). The subset of capacities used in Studies 2-3 are color-coded according to the domain they were selected to represent in those studies.", include = T}
knitr::include_graphics("../outputs/figS02_bodily-sensation_social-connection.pdf")
```

```{r figS04, fig.cap="Study 1, scatter plot visualizing the relationship between factor loadings on the factors we have labeled 'bodily sensation' and 'cognition and control' in the factor solution suggested by parallel analysis (loadings presented in Figure 1). The subset of capacities used in Studies 2-3 are color-coded according to the domain they were selected to represent in those studies.", include = T}
knitr::include_graphics("../outputs/figS02_bodily-sensation_cognition-and-control.pdf")
```

```{r figS05, fig.cap="Study 1, scatter plot visualizing the relationship between factor loadings on the factors we have labeled 'negative affect' and 'social connection' in the factor solution suggested by parallel analysis (loadings presented in Figure 1). The subset of capacities used in Studies 2-3 are color-coded according to the domain they were selected to represent in those studies.", include = T}
knitr::include_graphics("../outputs/figS02_negative-affect_cognition-and-control.pdf")
```

```{r figS06, fig.cap="Study 1, scatter plot visualizing the relationship between factor loadings on the factors we have labeled 'negative affect' and 'cognition and control' in the factor solution suggested by parallel analysis (loadings presented in Figure 1). The subset of capacities used in Studies 2-3 are color-coded according to the domain they were selected to represent in those studies.", include = T}
knitr::include_graphics("../outputs/figS02_negative-affect_cognition-and-control.pdf")
```

```{r figS07, fig.cap="Study 1, scatter plot visualizing the relationship between factor loadings on the factors we have labeled 'social connection' and 'cognition and control' in the factor solution suggested by parallel analysis (loadings presented in Figure 1). The subset of capacities used in Studies 2-3 are color-coded according to the domain they were selected to represent in those studies.", include = T}
knitr::include_graphics("../outputs/figS02_social-connection_cognition-and-control.pdf")
```
...
```{r figS08 code, fig.width = 9, fig.asp = 1.2}
...
```{r figS08, fig.cap="Study 1, factor solution suggested by minimizing BIC.", include = F}
knitr::include_graphics("../outputs/figS08.pdf")
```
...
```{r figS09 code, fig.width = 9, fig.asp = 1.2}
...
```{r figS09, fig.cap="Study 1, factor solution suggested by minimizing BIC.", include = F}
...
```{r figS10 code, fig.width = 8, fig.asp = 0.5}
d1_gam %>%
...
# save plot
if (resave_plots == T | !file.exists("../outputs/figS10.pdf")) {
  ggsave("../outputs/figS10.svg")
  ggsave("../outputs/figS10.pdf")
}
```

```{r figS10, fig.cap="Perceived developmental trajectories for four domains of mental life (Study 1). Lighter lines represent individual participants' scores, black points correspond to mean scores across the sample, and error bars are bootstrapped 95% confidence intervals.", include = T}
knitr::include_graphics("../outputs/figS10.pdf")
```
...
```{r figS11 code, fig.width = 9, fig.asp = 0.5}
...
# save plot
if (resave_plots == T | !file.exists("../outputs/figS11.pdf")) {
  ggsave("../outputs/figS11.svg")
  ggsave("../outputs/figS11.pdf")
}
```

```{r figS11, fig.cap="Study 2, factor solution suggested by parallel analysis. (Identical to Figure 1 in main text.)", include = F}
knitr::include_graphics("../outputs/figS11.pdf")
```
...
```{r figS12 code, fig.width = 9, fig.asp = 1.2}
heatmap_fun(s2_efa_bic)

# save plot
if (resave_plots == T | !file.exists("../outputs/figS12.pdf")) {
  ggsave("../outputs/figS12.svg")
  ggsave("../outputs/figS12.pdf")
}
```

```{r figS12, fig.cap="Study 2, factor solution suggested by minimizing BIC.", include = F}
knitr::include_graphics("../outputs/figS12.pdf")
```
...
# # alternatively, could define this based on study 1 data
# d2_scales <- d1_scales %>%
#   filter(capacity %in% colnames(d2_all))

# # alternatively, could define this based on study 2 data
...
```{r fig04, fig.cap="Network graph summarizing the importance of each developmental mechanism (along the bottom) for each capacity (along the top) among US adults (Study 3). Capacities are sorted and color-coded by domain (see Figures 2-3); developmental mechanisms are arranged in the fixed order of presentation (roughly, from what we perceived to be the most innate or biological mechanisms of development, to what we perceived to be the most learned or social). See main text for the full text of each mechanism. (Identical to Figure 4 in the main manuscript.)", fig.width = 8, fig.asp = 0.8}
d3_mech_ratings_graph <- d3_mech_ratings %>%
  mutate(dev_factor = as.character(dev_factor))

# create nodes dataframe
nodes <- tibble(
  name = c(d2_scales$capacity[d2_scales$capacity %in% unique(d3_mech_ratings$capacity)],
           levels(d3_mech_ratings$dev_factor))) %>%
  mutate(type = ifelse(name %in% d3_mech_ratings_graph$dev_factor, TRUE, FALSE))

# create bipartite graph
g <- graph_from_data_frame(
  d = d3_mech_ratings_graph %>%
    select(capacity, dev_factor, response) %>%
    # filter(!is.na(response), !is.na(dev_factor)) %>%
    group_by(capacity, dev_factor) %>%
    summarise(mean = mean(response, na.rm = T)) %>%
    ungroup(),
  vertices = nodes,
  directed = FALSE
...
ggraph(g, layout = "bipartite") +
  ...
  geom_node_point(
    data = . %>%
      mutate(name = factor(name, levels = nodes$name)),
    aes(color = name),
    size = 5) +
  geom_node_text(aes(label = gsub("_", " ", name)),
                 angle = 45,
                 # vjust = -2,
                 hjust = c(rep(0, 8), rep(1, 10)),
                 nudge_x = c(rep(0.05, 8), rep(-0.05, 10)),
                 nudge_y = c(rep(0.05, 8), rep(-0.05, 10)),
                 # nudge_y = c(rep(-1, 8), rep(1, 10)),
                 size = 3) +
  scale_color_manual(
    values = c(rep("#440154FF", 2),
               rep("#35608DFF", 2),
               rep("#22A884FF", 2),
               rep("#BBDF27FF", 2),
               c("#800080", "#8B1A6A", "#990033", "#A0002B", "#BD0026", 
                 "#D31B1C", "#E31A1C", "#F03B20", "#FC4E2A", "#FD8D3C"))
               # rep("#f94802", 10))
  ) +
  # scale_color_manual(
  #   values = c("FALSE" = "black", "TRUE" = "#f94802"),
  #   labels = c("Capacity", "Developmental mechanism"), name = "Node Type") +
  scale_edge_width_continuous(range = c(0.01, 0.7), limits = c(0, 6)) +
  scale_edge_alpha_continuous(range = c(0.05, 1), limits = c(0, 6), breaks = seq(0, 6, 1)) +
  scale_edge_colour_gradient2(
    # low = "gray95", high = "gray70",
    low = "gray90", mid = "gray80", high = "gray10",
    midpoint = 4,
    limits = c(0, 6),
    breaks = seq(0, 6, 1),
    labels = c("0 (not at all important)", 1:5, "6 (extremely important)")) +
  annotate("text", label = "Capacities", 
           x = 5.5, 
           y = Inf, vjust = 1, hjust = 0.5) +
  annotate("text", label = "Developmental mechanisms", 
           x = 5.5, 
           y = -Inf, vjust = -1, hjust = 0.5) +
  ...
  guides(edge_width = guide_none(),
         edge_alpha = guide_none(),
         edge_color = guide_edge_colorbar(barwidth = 0.5, barheight = 10),
         color = guide_none()) +
  labs(#title = "Two-Mode Network: Capacities & developmental mechanisms",
       # edge_width = "Mean rating:",
       # edge_alpha = "Mean rating:",
       edge_color = "Mean rating:\n")

# save plot
if (resave_plots == T | !file.exists("../outputs/fig04.pdf")) {
  ggsave("../outputs/fig04.jpg")
  ggsave("../outputs/fig04.svg")
  ggsave("../outputs/fig04.pdf")
}
```
...
```{r figS13 code, fig.width = 10, fig.asp = 1.2}
figS13 <- d123_gam %>%
...
if (resave_plots == T | !file.exists("../outputs/figS13.pdf")) {
  ggsave("../outputs/figS13.jpg")
  ggsave("../outputs/figS13.svg")
  ggsave("../outputs/figS13.pdf")
}
```

```{r figS13, fig.cap="Comparison of capacity ratings across studies (Studies 1-3). This visualization includes responses for each of the capacities and target ages held in common across at least two of our three studies. Lighter colorful points represent individual participants’ responses, larger points correspond to mean responses across the sample, and error bars are bootstrapped 95% confidence intervals.", include = T}
knitr::include_graphics("../outputs/figS13.pdf")
```


```{r}
d23_gam_pred <- d2_gam_r1_pred %>%
  mutate(study = "Study 2") %>%
  full_join(d3_gam_r1_pred %>% mutate(study = "Study 3")) %>%
  mutate(study = factor(study, levels = c("Study 1", "Study 2", "Study 3")))
```

```{r figS14 code, fig.width = 10, fig.asp = 0.5}
figS14 <- d23_gam_pred %>%
  full_join(
    d1_gam %>%
      mutate(study = "Study 1") %>%
      group_by(study, ResponseId, factor, target, target_year) %>%
      summarise(score = mean(response/100, na.rm = T))) %>%
  ggplot(aes(x = target_year, y = score, 
             color = study, fill = study)) +
  facet_grid(cols = vars(factor)) +
  geom_jitter(height = 0, width = 0.1, size = 0.01, alpha = 0.05) +
  geom_smooth(aes(y = pred_score, color = study, fill = study), 
              fill = NA, size = 0.8, lty = 1, 
              show.legend = F) +
  geom_path(
    data = . %>%
      filter(study == "Study 1") %>%
      # filter(target_year %in% c(0, 9/12, 5)) %>%
      group_by(study, target, target_year,
               factor) %>%
      summarise(mean = mean(score, na.rm = T)) %>%
      ungroup(),
    aes(y = mean, group = study),
  ...
  scale_y_continuous(
    ...
    labels = seq(0, 100, 25)) +
  # scale_x_discrete(labels = c("newborns", "9 months", "5 years")) +
  # scale_x_discrete(labels = levels(d123_gam$target_ord)) +
  ...
  theme(panel.background = element_rect(fill = NA), 
        ...
        # plot.margin = unit(c(0.5, 0.5, 0.5, 1.1), "cm")) +
  labs(x = "Target age (years)",
       y = "Mean score across all capacities in domain\n0 = not at all capable, 100 = completely capable",
       # caption = "Error bars are bootstrapped 95% CIs; solid lines are estimates from GAMs (Studies 1 and 2)",
       color = "Study:",
       fill = "Study:") +
  guides(color = guide_legend(override.aes = list(alpha = 1, size = 2)))
...
if (resave_plots == T | !file.exists("../outputs/figS14.pdf")) {
  ggsave("../outputs/figS14.jpg")
  ggsave("../outputs/figS14.svg")
  ggsave("../outputs/figS14.pdf")
}
```

```{r figS14, fig.cap="Comparison of mean scores across studies (Studies 1-3). These scores incorporate responses to all of the capacities and target ages included in each study; this varies across studies. Lighter points represent individual participants’ scores, larger points correspond to mean scores across the sample, and error bars are bootstrapped 95% confidence intervals. The dotted line for Study 1 connects mean scores across target ages; the solid lines for Studies 2-3 reflect predictions from study-specific GAMs.", include = T}
knitr::include_graphics("../outputs/figS14.pdf")
```
